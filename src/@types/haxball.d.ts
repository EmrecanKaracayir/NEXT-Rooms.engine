/**
 * Use this function to initialize the room. Returns a new instance of the
 * {@link Room} class.
 */
declare function HBInit(roomConfig: RoomConfig): Room;

/**
 * RoomConfig is passed to HBInit to configure the room, all values are
 * optional.
 */
interface RoomConfig {
  ...
  /**
   * Can be used to skip the recaptcha by setting it to a token that can be
   * obtained [here](https://www.haxball.com/headlesstoken).
   * @details
   * These tokens will expire after a few minutes.
   */
  token?: string;
  /**
   * If set to true the room player list will be empty, the {@link playerName}
...
/**
 * Room is the main interface which lets you control the room and listen to it's
 * events.
 */
declare class Room {
  /**
   ...
   * @details
   * If targetId is null or undefined the message is sent to all players. If
   * targetId is defined the message is sent only to the player with a matching
   * id.
   ...
   * does nothing. See example [here](https://github.com/haxball/haxball-issues/blob/master/headless/examples/setCustomStadium.js).
   */
  setCustomStadium(stadiumFileContents: string): void;

  /**
   * Sets the selected stadium to one of the default stadiums. The name must
  ...
  /**
   * Sets the teams lock. When teams are locked players are not able to change
   * team unless they are moved by an admin.
   */
  ...
  /**
   * Returns the player with the specified id. Returns null if the player
   * doesn't exist.
   */
  ...
  /**
   * Changes the password of the room, if pass is null the password will be
   * cleared.
   */
  ...
  /**
   * First all players listed are removed, then they are reinserted in the same
   * order they appear in the playerIdList.
   * @details
   * If moveToTop is true players are inserted at the top of the list, otherwise
   * they are inserted at the bottom of the list.
   */
  reorderPlayers(playerIdList: readonly number[], moveToTop: boolean): void;

  /**
   * Sends a host announcement with msg as contents. Unlike {@link sendChat},
   * announcements will work without a host player and has a larger limit on the
   * number of characters.
   * @param targetId If targetId is null or undefined the message is sent to all
   * players, otherwise it's sent only to the player with matching targetId.
   * @param color Will set the color of the announcement text, it's encoded as
   * an integer (0xFF0000 is red, 0x00FF00 is green, 0x0000FF is blue).
   * @param style style will set the style of the announcement text, it must be
   * one of the following strings: "normal","bold","italic", "small",
   * "small-bold", "small-italic"
   * @param If sound is set to 0 the announcement will produce no sound. If
   * sound is set to 1 the announcement will produce a normal chat sound. If set
   * to 2 it will produce a notification sound.
   */
  sendAnnouncement(
    msg: string,
    targetId?: number,
   ...
   * @param min The minimum number of logic-frames between two kicks. It is
   * impossible to kick faster than this.
   * @param rate Works like min but lets players save up extra kicks to use them
   * later depending on the value of burst.
   ...
   * @details
   * If avatar is set to null the override is cleared and the player will be
   * able to use his own avatar again.
   */
   ...
   * @details
   * If settings is set to null the override is cleared and the default stadium
   * settings will be used again. For example
   * `room.setDiscProperties(0, {x: 0, y: 0});` will set the position of disc 0
   * to <0,0> while leaving any other value intact.
   */
  setDiscProperties(
    discIndex: number,
  ...
  /**
   * Gets the properties of the disc at discIndex. Returns null if discIndex is
   * out of bounds.
   */
  getDiscProperties(discIndex: number): DiscProperties | null;

  /**
   * Same as {@link setDiscProperties} but targets the disc belonging to a
  ...
  /**
   * Same as {@link getDiscProperties} but targets the disc belonging to a
   * player with the given Id.
   */
  ...
  /**
   * Object filled with the collision flags constants that compose the cMask and
   * cGroup disc properties.
   * @details
   ...
   * @details The event function can return `false` in order to filter the chat
   * message. This prevents the chat message from reaching other players in the
   * room.
   */
   ...
   * @param byPlayer Is the player which caused the event (can be null if the
   * event wasn't caused by a player).
   */
  ...
  /**
   * @event onPlayerActivity called once for every game tick (happens 60 times
   * per second). This is useful if you want to monitor the player and ball
   ...
   * @details
   * After this event there's a timer before the game is fully unpaused, to
   * detect when the game has really resumed you can listen for the first
   * {@link onGameTick} event after this event is called.
   * @param byPlayer Is the player which caused the event (can be null if the
   * event wasn't caused by a player).
   */
  ...
  /**
   * @event onPositionsReset called when the players and ball positions are
   * reset after a goal happens.
   */
  onPositionsReset(): void;

  /**
   * @event onPlayerActivity called when a player gives signs of activity, such
   * as pressing a key.
   * @details
...
declare class Player {
  /**
   * The id of the player, each player that joins the room gets a unique id that
   * will never change.
   */
  id: number;
  /**
  ...
  admin: boolean;
  /**
   * The player's position in the field, if the player is not in the field the
   * value will be null.
   */
   ...
   * @details
   * The public ID is useful to validate that a player is who he claims to be,
   * but can't be used to verify that a player isn't someone else. Which means
   * it's useful for implementing user accounts, but not useful for implementing
   * a banning system.
   *
...
declare class Scores {
  ...
  blue: number;
  /**
   * The number of seconds elapsed (seconds don't advance while the game is
   * paused).
   */
  time: number;
  /**
...
declare class DiscProperties {
  ...
  damping: number;
  /**
   * The disc's color expressed as an integer (0xFF0000 is red, 0x00FF00 is
   * green, 0x0000FF is blue, -1 is transparent).
   */
  color: number;
  /**
   * The disc's cMask property. (Represents what groups the disc can collide
   * with).
   */
  cMask: number;
  /**
...
/**
 * Collision flags are what haxball's physics uses to determine which objects
 * collide and which objects ignore each other.
 * @details
...
declare class CollisionFlags {
  ...
  blue: number;
  /**
   * This layer represents kickoff barriers that become active during kickOff
   * for the red team.
   */
  redKO: number;
  /**
  ...
  wall: number;
  /**
   * Represents a set including {@link ball}, {@link red}, {@link blue},
   * {@link redKO}, {@link blueKO} and {@link wall}
   */
  all: number;
  /**
   * Objects with this flag in their {@link DiscProperties.cGroup} will become
   * kickable by the players.
   */
  kick: number;
  /**
   * Objects with this flag in their {@link DiscProperties.cGroup} will score
   * goals if they cross a goal line.
   */
  score: number;
  /**
